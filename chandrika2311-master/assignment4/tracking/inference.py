...
class ExactInference(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #print"belief-observe",self.beliefs
    ...
    def observe(self, observation, gameState):
        ...
        #print "jail posiiton",self.getJailPosition()
        #print "belief",self.beliefs
        #print "legal positions",self.legalPositions

        #Noisy Distance 3
        #emissionModel {1: 0.16492146596858642, 2: 0.16753926701570682, 3: 0.33507853403141363, 4: 0.16753926701570682, 
        #5: 0.08376963350785341, 6: 0.041884816753926704, 7: 0.020942408376963352, 8: 0.010471204188481676, 9: 0.005235602094240838, 
        #10: 0.002617801047120419}
        #pacmanPosition (5, 2)
        #belief {(5, 4): 0.125, (3, 4): 0.125, (1, 4): 0.125, (7, 4): 0.125, (9, 4): 0.125, (5, 2): 0.125, (2, 4): 0.125, (8, 4): 0.125}
        #legal positions [(1, 4), (2, 4), (3, 4), (5, 2), (5, 4), (7, 4), (8, 4), (9, 4)]
        ...
        if noisyDistance == None:
            probableBeliefs = util.Counter()
            probableBeliefs[self.getJailPosition()] = 1.0
        else:
        	probableBeliefs = util.Counter()
        	#print "probableBeliefs before", allPossible
        	for p in self.legalPositions:#print"distance",distance
                #print"distance",distance
                #print"distance",distance
        		probableBeliefs[p] = emissionModel[util.manhattanDistance(p, pacmanPosition)] * self.beliefs[p]
        		#print"distance",distance
        		#print "probableBeliefs[p]",probableBeliefs[p]
        		#print "emissionModel at each distance",emissionModel[distance]
        		#print"believ at each legal position"	,self.beliefs[p]
        #for p in self.legalPositions:
        #    trueDistance = util.manhattanDistance(p, pacmanPosition)
        #    if emissionModel[trueDistance] > 0:
        #        allPossible[p] = 1.0
			#print "probableBeliefs after", allPossible
        "*** END YOUR CODE HERE ***"

        probableBeliefs.normalize()
        self.beliefs = probableBeliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPosition in self.legalPositions:
        	newPosDistance = self.getPositionDistribution(self.setGhostPosition(gameState, oldPosition))
        	#print"legal positions:",self.legalPositions
        	#print"newposDist",newPosDistance
        	for newPos, prob in newPosDistance.items():
        		#print"newposDist",newPosDistance
        		probableBeliefs[newPos] = probableBeliefs[newPos] + prob * self.beliefs[oldPosition]
        self.beliefs = probableBeliefs		
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        assignedParticles= 0 #Note: the variable you store your particles in must be a list; a list is
        self.recreatingSamples = []#Note: the variable you store your particles in must be a list; a list is
        self.stateParticles =[]
        #Storing your particles as a Counter (where there could be an associated
        #weight with each position) is incorrect and may produce errors.
        while assignedParticles < self.numParticles:
       		for p in self.legalPositions:
        		#if the number of particles is less than the total particles append a new particle into this state
        		if assignedParticles < self.numParticles:
        			self.stateParticles.append(p)
        			assignedParticles += 1 #incrementing assigned particle count to record number of particles assigned till now
        #print  self.stateParticles           
    ...
    def observe(self, observation, gameState):
        ...
        #print"emissionModel", emissionModel
        #print"pacmanPosition",pacmanPosition
        #print"beliefDistribution",self.getBeliefDistribution()

        
        #import pdb; pdb.set_trace()
        if noisyDistance == None:
        	#When a ghost is captured by Pacman, all particles should be updated
            #so that the ghost appears in its prison cell,
            # self.getJailPosition()
            ...
            beliefOld = self.getBeliefDistribution()

            #calculating the distance between pacman and particl legal positions
            #calculating the distance between pacman and particl legal positions
            #calculating the distance between pacman and particl legal positions
            for position in self.legalPositions:#legal positions here are the particle positions
                distancebtPacPart = util.manhattanDistance(position, pacmanPosition)
                #calculating the distance between pacman and particl legal positions
                probableBeliefs[position] += emissionModel[distancebtPacPart] * beliefOld[position]
        	#self.beliefs = probableBeliefs--> Dont have to return belief now, instead return particles
        	#if there are no values in probableBeliefs, When all particles receive 0 weight, 
        	#they should be recreated from the prior distribution by calling initializeUniforml
        	if not any(probableBeliefs.values()):
        		self.initializeUniformly(gameState)
            else:
                recreatingSamples = []
                for i in range(0, self.numParticles):
                    recreatingSamples.append(util.sample(probableBeliefs)) 
                self.stateParticles = recreatingSamples
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.stateParticles:
            ...
            #to obtain the distribution over new positions for the ghost, given its
            #previous position (oldPos) as well as Pacman's current position.
            newDistribution.append(util.sample(newPosDist))
            #previous position (oldPos) as well as Pacman's current position.
        self.stateParticles = newDistribution
        #util.raiseNotDefined()
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        belief11 = util.Counter()
        #return the agent's current belief state, a distribution over ghost
        for i in self.stateParticles:
            #return the agent's current belief state, a distribution over ghost
        	belief11[i] += 1
        belief11.normalize()#locations conditioned on all evidence and time passage. This method
        return belief11	#locations conditioned on all evidence and time passage. This method
        #util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        #listOfParticles =  list(itertools.product(self.legalPositions))
        #finding cartesian product for each ghost, which is y ghost number of times repeated.
        listOfParticlepositions =  list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        #print"listOfParticles",listOfParticles
        #iterating over the legal positions for the total number of ghosts    
        #returnedpermutation need to be randomised
        random.shuffle(listOfParticlepositions)#returnedpermutation need to be randomised
        #print"randomizedPerm",randomizedPerm
        #returnedpermutation need to be randomised
        assignedParticles, self.stateParticles = 0, []
        self.recreatingSamples = []
        #if the number of particles is less than the total particles append a new particle into this state
        #if the number of particles is less than the total particles append a new particle into this state
        #if the number of particles is less than the total particles append a new particle into this state
        while assignedParticles < self.numParticles:
            for p in listOfParticlepositions:
                #if the number of particles is less than the total particles append a new particle into this state
                if assignedParticles < self.numParticles:
                    #if the number of particles is less than the total particles append a new particle into this state
                    #print"randomizedPerm",randomizedPerm
    ...
    def observeState(self, gameState):
        ...
        pacmanJagah = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        #import pdb; pdb.set_trace()
        probableBeliefs = util.Counter()#import pdb; pdb.set_trace()
        #range(len(self.ghostBeliefs)
        #calculating the distance between pacman and particl legal positions
        #calculating the distance between pacman and particl legal positions
        #calculating the distance between pacman and particl legal positions
        for content111, particle in enumerate(self.stateParticles):
            #Storing a temporary variable to get tvalues later
            temporaryVariable = 1.0
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    #When a ghost is captured by Pacman, all particles should be updated                    
                    particle = self.getParticleWithGhostInJail(particle, i)#o that the ghost appears in its prison cell, position
                    #self.getJailPosition(i) where `i` is the index of the ghost.
                else:
                    doori = util.manhattanDistance(particle[i], pacmanJagah)
                    temporaryVariable *= emissionModels[i][doori]
                    #updating the beliefs using the temporary variable
            probableBeliefs[particle] += temporaryVariable
        ...
        if not any(probableBeliefs.values()):
            self.initializeParticles()
            #calculating the distance between pacman and particl legal positions
            for content22, particle in enumerate(self.stateParticles):
                for i in range(self.numGhosts):
                    if noisyDistances[i] == None:
                        #calculating the distance between pacman and particl legal positions
            ...
            #doing the recreation of belief state after all became zero
            probableBeliefs.normalize()
            recreatingSamples = []
            for holloo in range(0, self.numParticles):
                #recreatingSamplesrecreatingSamples
                recreatingSamples.append(util.sample(probableBeliefs))
                #recreatingSamplesrecreatingSamples
            self.stateParticles = recreatingSamples
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.stateParticles:
            ...
            "*** YOUR CODE HERE ***"
            for m in range(self.numGhosts):
                ghostpositiondist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                   m, self.ghostAgents[m])
                #getPositionDistributionForGhost(gameState, ghostIndex, agent)
                newParticle[m] = util.sample(ghostpositiondist)
                #getPositionDistributionForGhost(gameState, ghostIndex, agent)
            "*** END YOUR CODE HERE ***"
