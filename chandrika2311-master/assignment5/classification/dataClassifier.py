...
def enhancedFeatureExtractorDigit(datum):
    ...
    for i in range(1, DIGIT_DATUM_WIDTH):
        for j in range(1, DIGIT_DATUM_HEIGHT):
            pixel1 = datum.getPixel(i, j)
            pixel2 = datum.getPixel(i - 1, j)
            pixel3 = datum.getPixel(i, j)
            pixel4 = datum.getPixel(i, j - 1)
            features[("horiz", i, j)] = int(pixel1 >
                                            pixel2)
            features[("verti", i, j)] = int(pixel3 >
                                            pixel4)
            if datum.getPixel(i,j) == 0 and (i,j) not in visited:
                ...
                if queue:
                    ...
                    (x, y) = point
                    for each in get_points(*point) :
                        #print "point", point
                        if (datum.getPixel(*each)) < 1 and each not in visited:
                            queue.append(each)
                number_of_regions += 1
            

    features[1] = number_of_regions % 2
    features[2] = (number_of_regions >> 1) % 2
    features[3] = (number_of_regions >> 2)% 2
...
def enhancedPacmanFeatures(state, action):
    ...
    foodPellets = successor.getFood().asList()
    PowerPellets = successor.getCapsules()
    foodleft = successor.getNumFood()
    nearestFooddist = None
    if foodPellets:
        for food in foodPellets:
            foodDistance = util.manhattanDistance(pacmanPosition, food)
            if foodDistance < nearestFooddist or nearestFooddist == None:
                nearestFooddist = foodDistance
    
        if not nearestFooddist == 0 or nearestFooddist == None:
            features['nearestFoodDistance'] = 3/nearestFooddist
        else:
            features['nearestFoodDistance'] = 0

    nearestGhostDistance = None
    for ghost in ghostPosition:
        GhostDistance = util.manhattanDistance(pacmanPosition, ghost)
        if GhostDistance < nearestGhostDistance or nearestGhostDistance == None:
                nearestGhostDistance = GhostDistance

    if not nearestGhostDistance == 0 or nearestGhostDistance == None:
        features['nearestGhostDistance'] = 10/nearestGhostDistance 
    else:
        features['nearestGhostDistance'] = -10
    nearestCapsuleDistance = None
    
    
    features["numberOfFoodPellets"] = len(foodPellets)
